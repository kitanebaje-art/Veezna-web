...
export interface Course {
  ...
  description: string;
  duration: string;
  fee: number; // Course / Tuition Fee
  registrationFee: number;
  totalFee: number; // Automatically normalized fee = fee + registrationFee
  tuitionFee?: number; // Maintained for backward compatibility if present in old docs
  totalSeats: number;
...
export default function AdminCoursesPage() {
  ...
  const fetchCourses = async () => {
    ...
    try {
      ...
      const courseList: Course[] = snapshot.docs.map((docSnap) => {
        ...
        // Backward-compatible Fee Extraction
        const rawTuition = parseNumber(data.fee ?? data.tuitionFee);
        const rawReg = parseNumber(data.registrationFee);
        const rawTotal = parseNumber(data.totalFee);

        // Normalize fees safely so old and new schemas always resolve cleanly
        const tuitionFee = rawTuition;
        const registrationFee = rawReg;
        const totalFee = rawTotal > 0 && rawReg === 0 && rawTuition === 0
          ? rawTotal // If only old totalFee existed
      ...
      // Sort client-side safely by createdAt desc
      courseList.sort((a, b) => {
  ...
  // Calculated Live Total for Form Inputs
  const liveTuitionFee = Math.max(0, parseNumber(formData.fee));
  const liveRegistrationFee = Math.max(0, parseNumber(formData.registrationFee));
  const liveTotalFee = liveTuitionFee + liveRegistrationFee;
  ...
  const openEditModal = (course: Course) => {
    ...
    setFormData({
      ...
      registrationFee: course.registrationFee !== undefined ? String(course.registrationFee) : "",
      totalSeats: course.totalSeats !== undefined ? String(course.totalSeats) : "",
      availableSeats: course.availableSeats !== undefined ? String(course.availableSeats) : "",
  ...
  // Submit Handler with Strict Fee Validation & Normalization
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const rawTuition = parseNumber(formData.fee);
    const rawReg = parseNumber(formData.registrationFee);
    const totalSeatsNum = parseNumber(formData.totalSeats);
    const availSeatsNum = parseNumber(formData.availableSeats);

    if (rawTuition < 0) {
      alert("Course / Tuition Fee cannot be negative.");
      return;
    }
    ...
    if (totalSeatsNum < 0 || availSeatsNum < 0) {
      alert("Seats quantity cannot be negative.");
      return;
    }
    ...
    // Always calculate numeric fee, registrationFee, and totalFee
    const tuitionFee = Math.max(0, rawTuition);
    const registrationFee = Math.max(0, rawReg);
    ...
    try {
      if (editingCourse) {
        ...
        const updatePayload = {
          ...
          fee: tuitionFee,
          tuitionFee: tuitionFee, // duplicate for backward compatibility
          registrationFee: registrationFee,
          totalFee: totalFee,
              ...
              return (
                ...
                >
                  <div className="space-y-3">
                    ...
                    {/* FEE STRUCTURE CARD DISPLAY */}
                    <div className="grid grid-cols-3 gap-1.5 text-xs pt-3 border-t border-slate-100 bg-slate-50 p-3 rounded-xl">
                      <div>
                        <span className="text-slate-400 block text-[9px] font-bold uppercase truncate">Course Fee</span>
                        <span className="text-slate-700 font-semibold text-xs">{formatCurrency(course.fee)}</span>
                      </div>
                      <div>
                        <span className="text-slate-400 block text-[9px] font-bold uppercase truncate">Registration</span>
                        <span className="text-slate-700 font-semibold text-xs">{formatCurrency(course.registrationFee)}</span>
                      </div>
                      <div className="bg-blue-50/80 border border-blue-100 p-1.5 rounded-lg">
                        <span className="text-[#0057B8] block text-[9px] font-extrabold uppercase truncate">Total Fee</span>
                        <span className="text-[#0057B8] font-extrabold text-xs">{formatCurrency(course.totalFee)}</span>
                      </div>
                    </div>
            ...
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-2 text-xs">
              ...
              <div className="grid grid-cols-3 gap-2 text-center pt-1">
                <div className="bg-white p-2.5 rounded-xl border border-slate-200">
                  ...
                  <span className="font-bold text-slate-800 text-xs">{formatCurrency(viewingCourse.fee)}</span>
                </div>
                <div className="bg-white p-2.5 rounded-xl border border-slate-200">
                ...
                <div className="bg-blue-50 p-2.5 rounded-xl border border-blue-200">
                  <span className="block text-[9px] text-[#0057B8] font-extrabold uppercase">Total Fee</span>
                  <span className="font-extrabold text-[#0057B8] text-xs">
                    {formatCurrency(viewingCourse.totalFee)}
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-200 p-6 sm:p-8 space-y-5 relative my-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              ...
              {/* SEPARATED & VALIDATED FEE INPUTS */}
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-600 font-semibold mb-1">Course / Tuition Fee (₹)</label>
                  <input
                    type="number"
                    min="0"
                    value={formData.fee}
              ...
              <div className="p-3 bg-[#0057B8]/5 border border-[#0057B8]/20 rounded-xl space-y-1">
                <span className="text-[10px] uppercase font-bold text-[#0057B8] block mb-1">Live Fee Structure Preview</span>
                <div className="flex justify-between items-center text-xs text-slate-600">
                  <span>Tuition Fee:</span>
                  <span className="font-semibold">{formatCurrency(liveTuitionFee)}</span>
                </div>
                <div className="flex justify-between items-center text-xs text-slate-600">
                  <span>Registration Fee:</span>
                  <span className="font-semibold">{formatCurrency(liveRegistrationFee)}</span>
                </div>
                <div className="flex justify-between items-center text-xs font-bold text-[#0057B8] pt-1 border-t border-[#0057B8]/20">
                  <span>Total Course Fee:</span>
                  <span className="text-sm">{formatCurrency(liveTotalFee)}</span>
                </div>
              </div>
